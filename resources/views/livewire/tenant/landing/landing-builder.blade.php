...
--}}
<div
    class="flex flex-col h-screen overflow-hidden text-[#f0f0f8]"
...
{{-- ╔══════════════════════════════════════════╗
     ║  TOPBAR                                  ║
     ╚══════════════════════════════════════════╝ --}}
<header class="flex items-center justify-between px-6 h-14 flex-shrink-0 border-b gap-4"
        style="background:#13131a; border-color:rgba(255,255,255,0.07)">

    <div class="flex items-center gap-3">
        <a href="{{ route('dashboard') }}" class="flex items-center gap-2 text-sm font-bold text-white/90 hover:text-white transition-colors">
            <div class="size-7 rounded-lg flex items-center justify-center flex-shrink-0"
                 style="background:{{ $colorPrimary }}">
                <svg class="size-[14px]" fill="none" stroke="white" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
                    <path d="M13 2 3 14h9l-1 8 10-12h-9z"/>
                </svg>
            </div>
            SaaSFlow
        </a>
        <span class="w-px h-5" style="background:rgba(255,255,255,0.1)"></span>
        <span class="text-sm font-medium" style="color:rgba(255,255,255,0.35)">Landing Page</span>
    </div>
    ...
    <div class="flex items-center gap-2">
        ...
        <a href="{{ route('tenant.landing.preview') }}" target="_blank"
           class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all"
           style="color:rgba(255,255,255,0.45); border:1px solid rgba(255,255,255,0.1); background:transparent;"
           onmouseover="this.style.background='rgba(255,255,255,0.05)'; this.style.color='rgba(255,255,255,0.75)'"
...
<div class="flex flex-1 overflow-hidden">
    ...
    <aside class="w-[360px] flex-shrink-0 flex flex-col overflow-hidden"
           style="background:#13131a; border-right:1px solid rgba(255,255,255,0.07)">

        {{-- Tabs --}}
        <nav class="flex px-1 gap-0.5 border-b flex-shrink-0" style="border-color:rgba(255,255,255,0.07)">
            @foreach([
                'template' => 'Plantilla',
                'sections' => 'Secciones',
                'style'    => 'Estilo',
                ...
                <button wire:click="$set('activeTab', '{{ $tab }}')"
                        ...
                        style="{{ $activeTab === $tab
                            ? 'color:#9d93ff; border-color:#7c6ff7;'
                            : 'color:rgba(255,255,255,0.25); border-color:transparent;' }}"
                >{{ $label }}</button>
            @endforeach
        </nav>

        {{-- Scrollable body --}}
        <div class="flex-1 overflow-y-auto" style="scrollbar-width:thin; scrollbar-color:rgba(255,255,255,0.08) transparent">

            {{-- ══════════════ TAB: PLANTILLA ══════════════ --}}
            @if($activeTab === 'template')
            <div class="p-5">
                <p class="text-[10px] font-bold uppercase tracking-widest mb-3" style="color:rgba(255,255,255,0.25)">
                ...
                <div class="grid grid-cols-2 gap-2 mb-5">
                    ...
                    <button
                        ...
                        style="border:2px solid {{ $templateKey === $tpl['key'] ? '#7c6ff7' : 'rgba(255,255,255,0.07)' }};
                               {{ $templateKey === $tpl['key'] ? 'box-shadow:0 0 0 1px #7c6ff7, 0 8px 24px rgba(124,111,247,0.2)' : '' }}"
                    >
                        ...
                        @switch($tpl['key'])
                            ...
                            <div class="w-full h-full flex flex-col" style="background:#fafafa">
                                <div class="flex items-center justify-between px-2 py-1.5" style="background:{{ $colorPrimary }}">
                                    <div class="h-1 w-5 rounded-full" style="background:rgba(255,255,255,0.7)"></div>
                        ...
                        <div class="absolute inset-0 flex flex-col justify-end p-2 transition-opacity {{ $templateKey === $tpl['key'] ? 'opacity-100' : 'opacity-0 group-hover:opacity-100' }}"
                            ...
                            <p class="text-[9px]" style="color:rgba(255,255,255,0.5)">{{ $tpl['vibe'] ?? ($tpl['description'] ?? '') }}</p>
                        </div>
                ...
                <div class="flex items-center gap-3 p-3 rounded-xl" style="background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.06)">
                    <div class="size-9 rounded-lg flex items-center justify-center flex-shrink-0" style="background:rgba(124,111,247,0.15)">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-[10px]" style="color:rgba(255,255,255,0.3)">Plantilla activa</p>
                    </div>
                    <span class="text-[10px] font-bold px-2 py-0.5 rounded-full" style="background:rgba(124,111,247,0.15); color:#9d93ff">Activa</span>
                </div>
            </div>
            @endif
            ...
            <div class="p-5">
                <p class="text-[10px] font-bold uppercase tracking-widest mb-1" style="color:rgba(255,255,255,0.25)">Secciones</p>
                <p class="text-[11px] mb-4" style="color:rgba(255,255,255,0.3)">Arrastra · activa/desactiva · edita</p>
                ...
                <div class="space-y-1.5 mb-3">
                    ...
                    <div
                        ...
                        style="border:1px solid {{ $selectedBlockId === $block->id ? 'rgba(124,111,247,0.5)' : 'rgba(255,255,255,0.06)' }};
                               background:{{ $selectedBlockId === $block->id ? 'rgba(124,111,247,0.07)' : 'rgba(255,255,255,0.02)' }};
                               opacity:{{ $block->is_active ? '1' : '0.4' }}"
                        wire:click="$set('selectedBlockId', {{ $selectedBlockId === $block->id ? 'null' : $block->id }})"
                    >
                        {{-- Drag handle --}}
                        <div class="flex-shrink-0 cursor-grab active:cursor-grabbing" style="color:rgba(255,255,255,0.2)">
                        ...
                        {{-- Info --}}
                        <div class="flex-1 min-w-0">
                            <p class="text-xs font-semibold text-white leading-none truncate">{{ $block->getLabel() }}</p>
                            <p class="text-[10px] mt-0.5 truncate" style="color:rgba(255,255,255,0.3)">{{ $block->getTag() }}</p>
                        </div>
                        ...
                        <button wire:click.stop="openEditPanel({{ $block->id }})"
                                ...
                                onmouseover="this.style.color='rgba(255,255,255,0.8)'; this.style.background='rgba(255,255,255,0.08)'"
                ...
                <button wire:click="$set('activeTab', 'add')"
                        ...
                        onmouseover="this.style.borderColor='rgba(124,111,247,0.5)'; this.style.color='#9d93ff'; this.style.background='rgba(124,111,247,0.05)'"
                        onmouseout="this.style.borderColor='rgba(255,255,255,0.1)'; this.style.color='rgba(255,255,255,0.3)'; this.style.background='transparent'">
            ...
            <div class="p-5 space-y-5">
                ...
                {{-- Nombre del sitio --}}
                <div>
                    <p class="text-[10px] font-bold uppercase tracking-widest mb-2" style="color:rgba(255,255,255,0.25)">Nombre del sitio</p>
                    <input wire:model.blur="siteName" type="text"
                           class="w-full px-3 py-2 rounded-lg text-sm text-white placeholder-white/25 focus:outline-none transition-colors"
                ...
                {{-- 3 Color pickers --}}
                <div>
                    <p class="text-[10px] font-bold uppercase tracking-widest mb-3" style="color:rgba(255,255,255,0.25)">Colores</p>
                    ...
                    <div class="mb-3">
                        <div class="flex items-center justify-between mb-1.5">
                            <span class="text-xs font-semibold text-white">{{ $c['label'] }}</span>
                            <span class="text-[10px]" style="color:rgba(255,255,255,0.3)">{{ $c['hint'] }}</span>
                        </div>
                        ...
                        <button @click="activePicker = activePicker === '{{ $c['alpine'] }}' ? null : '{{ $c['alpine'] }}'"
                            ...
                            <span class="font-mono text-xs text-white flex-1 text-left">{{ strtoupper(${'color'.ucfirst($c['alpine'])}) }}</span>
                        ...
                        <div x-show="activePicker === '{{ $c['alpine'] }}'"
                            ...
                            <input type="text"
                                   ...
                                   class="font-mono text-[10px] px-2 py-1 rounded-lg text-white focus:outline-none"
                                   style="width:76px; background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.1)"
                ...
                {{-- Tipografía --}}
                <div>
                    <p class="text-[10px] font-bold uppercase tracking-widest mb-2" style="color:rgba(255,255,255,0.25)">Tipografía</p>
                    <div class="grid grid-cols-2 gap-1.5">
                        ...
                        <button wire:click="$set('fontFamily', '{{ $font['key'] }}')"
                                ...
                                style="border:1px solid {{ $fontFamily === $font['key'] ? 'rgba(124,111,247,0.6)' : 'rgba(255,255,255,0.06)' }};
                                       background:{{ $fontFamily === $font['key'] ? 'rgba(124,111,247,0.08)' : 'rgba(255,255,255,0.02)' }}">
                            <p class="text-sm font-bold leading-none mb-0.5"
                               style="color:{{ $fontFamily === $font['key'] ? '#9d93ff' : 'rgba(255,255,255,0.7)' }}">{{ $font['name'] }}</p>
                            <p class="text-[9px]" style="color:rgba(255,255,255,0.3)">{{ $font['sample'] }}</p>
                        </button>
                        @endforeach
                    </div>
                </div>
                ...
                {{-- Fondo --}}
                <div>
                    <p class="text-[10px] font-bold uppercase tracking-widest mb-2" style="color:rgba(255,255,255,0.25)">Estilo de fondo</p>
                    <div class="grid grid-cols-3 gap-1.5">
                        ...
                        <button wire:click="$set('bgMode', '{{ $key }}')"
                                ...
                                style="border:1px solid {{ $bgMode === $key ? 'rgba(124,111,247,0.6)' : 'rgba(255,255,255,0.06)' }};
                                       background:{{ $bgMode === $key ? 'rgba(124,111,247,0.08)' : 'rgba(255,255,255,0.02)' }}">
                ...
                {{-- Paletas rápidas --}}
                <div>
                    <p class="text-[10px] font-bold uppercase tracking-widest mb-2" style="color:rgba(255,255,255,0.25)">Paletas listas</p>
                    <div class="grid grid-cols-2 gap-1.5" x-data>
                        <template x-for="p in quickPalettes" :key="p.name">
                            <button @click="$wire.applyPalette(p.primary, p.neutral, p.accent)"
                                    ...
                                    :style="{
                                        border: '1px solid ' + ('{{ $colorPrimary }}' === p.primary ? p.primary + '50' : 'rgba(255,255,255,0.06)'),
                                        background: '{{ $colorPrimary }}' === p.primary ? p.primary + '12' : 'rgba(255,255,255,0.02)',
                                    }">
                                ...
                                <div>
                                    <p class="text-[11px] font-bold text-white" x-text="p.name"></p>
                                    <p class="text-[10px]" style="color:rgba(255,255,255,0.3)" x-text="p.vibe"></p>
                                </div>
                            </button>
                        </template>
                    </div>
        ...
        {{-- Footer --}}
        <div class="flex gap-2 p-4 flex-shrink-0" style="border-top:1px solid rgba(255,255,255,0.07)">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        ...
        <div class="flex items-center justify-between px-5 py-2.5 flex-shrink-0"
             style="background:#13131a; border-bottom:1px solid rgba(255,255,255,0.07)">

            {{-- Viewport --}}
            <div class="flex p-1 gap-0.5 rounded-xl" style="background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.07)">
                @foreach(['desktop' => 'Escritorio', 'tablet' => 'Tablet', 'mobile' => 'Móvil'] as $vp => $label)
                <button wire:click="$set('viewport', '{{ $vp }}')"
                        class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all"
                        style="{{ $viewport === $vp
                            ? 'background:rgba(255,255,255,0.08); color:white; box-shadow:0 1px 4px rgba(0,0,0,0.3)'
            ...
            <div class="text-xs" style="color:rgba(255,255,255,0.3)">
                ...
                <span class="font-semibold" style="color:rgba(255,255,255,0.5)">
        ...
        <div class="flex-1 overflow-auto flex items-start justify-center p-8"
             style="background: radial-gradient(circle at 20% 20%, {{ $colorPrimary }}06, transparent 50%), radial-gradient(circle at 80% 80%, {{ $colorAccent }}04, transparent 40%), #0e0e13;
                    background-image: radial-gradient(rgba(255,255,255,0.04) 1px, transparent 1px);
                    background-size: 24px 24px;">

            {{-- Preview window --}}
            <div class="bg-white rounded-2xl overflow-hidden transition-all duration-300 w-full"
                 style="max-width: {{ $viewport === 'mobile' ? '390px' : ($viewport === 'tablet' ? '768px' : '900px') }};
                        box-shadow: 0 0 0 1px rgba(255,255,255,0.08), 0 32px 80px rgba(0,0,0,0.6)">
...
<div class="fixed top-14 right-0 bottom-0 w-80 flex flex-col z-40 transition-transform duration-300 {{ $editPanelOpen ? 'translate-x-0' : 'translate-x-full' }}"
     style="background:#13131a; border-left:1px solid rgba(255,255,255,0.1); box-shadow:-8px 0 32px rgba(0,0,0,0.3)">
    ...
    <div class="flex items-center justify-between px-4 py-3 flex-shrink-0" style="border-bottom:1px solid rgba(255,255,255,0.07)">
        <p class="text-sm font-bold text-white">
            @if($editingBlock){{ $editingBlock->getEmoji() }} {{ $editingBlock->getLabel() }}@endif
        </p>
        <button wire:click="closeEditPanel" class="size-7 flex items-center justify-center rounded-lg transition-all"
                style="color:rgba(255,255,255,0.3)"
                onmouseover="this.style.color='white'; this.style.background='rgba(255,255,255,0.06)'"
                onmouseout="this.style.color='rgba(255,255,255,0.3)'; this.style.background='transparent'">
    ...
    {{-- Settings form --}}
    <div class="flex-1 overflow-y-auto p-4" style="scrollbar-width:thin; scrollbar-color:rgba(255,255,255,0.08) transparent">
        @if($editingBlock)
            @includeIf('livewire.tenant.landing.block-settings.' . $editingBlock->block_type, [
                'block'    => $editingBlock,
    ...
    {{-- Save --}}
    <div class="p-4 flex-shrink-0" style="border-top:1px solid rgba(255,255,255,0.07)">
...
<div
    ...
    :class="{
        ...
        'text-amber-300':   type==='info',
        'text-red-300':     type==='error',
    }"
    style="display:none; background:#1e1e2e; border:1px solid rgba(255,255,255,0.1); box-shadow:0 8px 32px rgba(0,0,0,0.4); white-space:nowrap"
>
